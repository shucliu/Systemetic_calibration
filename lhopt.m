function [PSopt xstar xopt pi]=lhopt(metamodel,parameters,datamatrix,lhacc)
...
for i=1:N
  UB(i)=range{i}(2); %upper boundary max_parameter
  LB(i)=range{i}(1); %lower boundary min_parameter
end
...
cnt=0;
st=1000;
pi = zeros(sd(1),sd(2),sd(4),length(xstar));
%dmatrix = zeros(sd(1),sd(2),sd(3),sd(4),length(xstar));
