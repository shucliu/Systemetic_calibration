...
for i=1:size(pqn,1)
  % Minimum / Minimum
  paramlist(2*N+i,pqn(i,1))=range{pqn(i,1)}(1);
  paramlist(2*N+i,pqn(i,2))=range{pqn(i,2)}(1);
  
  % Minimum / Maximum
  paramlist(2*N+i+size(pqn,1),pqn(i,1))=range{pqn(i,1)}(1);
  paramlist(2*N+i+size(pqn,1),pqn(i,2))=range{pqn(i,2)}(2);
  
  % Maximum / Minimum
  paramlist(2*N+i+size(pqn,1)*2,pqn(i,1))=range{pqn(i,1)}(2);
  paramlist(2*N+i+size(pqn,1)*2,pqn(i,2))=range{pqn(i,2)}(1);
  
  % Maximum / Maximum
  paramlist(2*N+i+size(pqn,1)*3,pqn(i,1))=range{pqn(i,1)}(2);
  paramlist(2*N+i+size(pqn,1)*3,pqn(i,2))=range{pqn(i,2)}(2);
end

for i=1:nlhs
  paramlist(4*size(pqn,1)+2*N+i,:)=lhsdata{i};
end
